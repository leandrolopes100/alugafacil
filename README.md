# Aluga Fácil

SaaS multi-tenant para locadoras de veículos. Cada locadora opera em seu próprio schema PostgreSQL isolado, com controle de frota, contratos, financeiro e manutenção em um único sistema.

---

## Stack

| Camada | Tecnologia |
|---|---|
| Backend | Django 5.2 · Python 3.12 |
| Banco de dados | PostgreSQL (obrigatório) |
| Multi-tenant | django-tenants 3.10.1 (schema-based) |
| Frontend | Bootstrap 5 · Alpine.js · HTMX |
| Tarefas assíncronas | Celery 5.5 + Redis |
| Arquivos estáticos | Whitenoise |
| Storage em produção | django-storages + boto3 (S3) |
| PDF | xhtml2pdf |
| Variáveis de ambiente | python-decouple |

---

## Arquitetura multi-tenant

O sistema usa **isolamento por schema** do PostgreSQL via `django-tenants`. Cada locadora cadastrada recebe seu próprio schema, garantindo separação total dos dados.

```
schema: public
  └── apps.tenants  (TenantCompany, Domain)
  └── apps.core     (relatórios, dashboard, busca global)

schema: <slug_da_locadora>
  └── apps.fleet        (frota de veículos)
  └── apps.customers    (clientes e CNH)
  └── apps.contracts    (reservas e contratos)
  └── apps.financeiro   (contas, despesas, multas)
  └── apps.manutencao   (ordens de serviço e alertas)
  └── apps.investidores (gestão de investidores)
```

### Planos disponíveis

| Plano | Limite de veículos |
|---|---|
| Básico | Até 10 |
| Profissional | Até 30 |
| Premium | Ilimitado |

---

## Módulos

### Frota (`fleet`)
- Categorias e grupos de veículos com tarifas (diária, semanal, mensal, franquia km, caução)
- Situações do veículo: `disponível` · `reservado` · `em uso` · `manutenção` · `inativo`
- Fotos e documentos (CRLV, seguro, extintor) com alertas de vencimento
- Histórico de quilometragem por contrato

### Clientes (`customers`)
- Pessoas físicas e jurídicas
- CNH com categoria, validade e fotos frente/verso
- Situação: `ativo` · `bloqueado`
- Busca por nome, CPF/CNPJ ou celular

### Contratos (`contracts`)
- Fluxo completo: **Reserva → Contrato → Checkout → Checkin → Encerramento**
- Numeração automática `AF-YYYY-NNNN`
- Situações: `aberto` · `ativo` · `aguardando_devolucao` · `encerrado` · `cancelado`
- Checklist de checkout com bloqueadores (documentos vencidos, caução pendente)
- Parcelas semanais upfront com fechamento automático FIFO ao receber pagamento
- Adicionais (GPS, cadeirinha, condutor extra, seguros) e registro de avarias
- Assinatura digital por link (token UUID)
- Properties calculadas: `total_locacao` · `total_adicionais` · `total_avarias` · `total_geral` · `total_pago` · `saldo_devedor`

### Financeiro (`financeiro`)
- Contas a receber vinculadas automaticamente a contratos via signal
- Despesas operacionais com parcelamento e débito automático
- Multas de trânsito com auto-associação por período do contrato
- Configuração de multa/juros por atraso, carência e custo de reposição de combustível

### Manutenção (`manutencao`)
- Ordens de serviço: preventiva · corretiva · sinistro
- Alertas por quilometragem ou data

### Investidores (`investidores`)
- Cadastro de investidores PF/PJ com dados bancários
- Vínculo veículo ↔ investidor com taxa de gestão semanal
- Cobranças de gestão individuais ou em lote
- Badge na sidebar para cobranças vencidas

### Core (`core`)
- Dashboard com visão geral da frota, devoluções do dia e alertas
- Relatórios: Contratos · Frota/Ocupação · DRE · Clientes · Inadimplência
- Exportação CSV
- Busca global

---

## Grupos de permissão

| Grupo | Acesso |
|---|---|
| `admin_locadora` | Total — todos os módulos |
| `atendente` | Clientes, reservas e contratos (sem exclusão) |
| `financeiro` | Módulo financeiro e relatórios (sem exclusão) |
| `mecanico` | Manutenção e frota (sem exclusão) |

---

## Configuração do ambiente

### 1. Pré-requisitos

- Python 3.12
- PostgreSQL 14+
- Redis (para Celery)

### 2. Clonar e criar o ambiente virtual

```bash
git clone <repositorio>
cd "ALUGA FÁCIL"
python -m venv venv
venv\Scripts\activate        # Windows
# source venv/bin/activate   # Linux/Mac
pip install -r requirements.txt
```

### 3. Variáveis de ambiente

Crie um arquivo `.env` na raiz do projeto:

```env
SECRET_KEY=sua-chave-secreta-aqui
DEBUG=True

DB_NAME=alugafacil
DB_USER=alugafacil
DB_PASSWORD=sua-senha
DB_HOST=localhost
DB_PORT=5432

REDIS_URL=redis://localhost:6379/0

# Produção (opcional)
AWS_ACCESS_KEY_ID=
AWS_SECRET_ACCESS_KEY=
AWS_STORAGE_BUCKET_NAME=
```

### 4. Banco de dados

```bash
# Criar o banco no PostgreSQL
psql -U postgres -c "CREATE DATABASE alugafacil;"
psql -U postgres -c "CREATE USER alugafacil WITH PASSWORD 'sua-senha';"
psql -U postgres -c "GRANT ALL PRIVILEGES ON DATABASE alugafacil TO alugafacil;"

# Aplicar migrations no schema público
python manage.py migrate_schemas --shared

# Criar o primeiro tenant (locadora)
python manage.py shell
```

```python
from apps.tenants.models import TenantCompany, Domain

tenant = TenantCompany.objects.create(
    schema_name='locadora1',
    name='Minha Locadora',
    slug='locadora1',
    plan='professional',
)
Domain.objects.create(tenant=tenant, domain='locadora1.localhost', is_primary=True)
```

```bash
# Aplicar migrations no schema do tenant
python manage.py migrate_schemas --tenant
```

### 5. Grupos de permissão e superusuário

```bash
# Criar grupos (rodar dentro do schema do tenant)
python manage.py tenant_command criar_grupos --schema=locadora1

# Criar superusuário
python manage.py tenant_command createsuperuser --schema=locadora1
```

### 6. Dados de demonstração (opcional)

```bash
python manage.py tenant_command popular_dados_teste --schema=locadora1
# Para limpar antes de repopular:
python manage.py tenant_command popular_dados_teste --schema=locadora1 --limpar
```

O comando cria 10 veículos, 8 clientes, 8 contratos em todos os estados, reservas, pagamentos, despesas, multas, ordens de manutenção e usuários com todos os grupos.

### 7. Arquivos estáticos

```bash
python manage.py collectstatic
```

### 8. Iniciar o servidor

```bash
# Django dev server
python manage.py runserver

# Celery (em terminal separado)
celery -A config worker -l info
```

Acesse pelo domínio configurado no tenant, ex: `http://locadora1.localhost:8000`

---

## Estrutura do projeto

```
ALUGA FÁCIL/
├── apps/
│   ├── tenants/        # Modelos TenantCompany e Domain
│   ├── core/           # Dashboard, relatórios, mixins, middleware
│   ├── fleet/          # Frota de veículos
│   ├── customers/      # Clientes e CNH
│   ├── contracts/      # Reservas, contratos e pagamentos
│   ├── financeiro/     # Contas, despesas e multas
│   ├── manutencao/     # Ordens de serviço e alertas
│   └── investidores/   # Gestão de investidores
├── config/
│   ├── settings/
│   │   ├── base.py
│   │   └── development.py
│   ├── urls.py         # Schema público
│   ├── tenant_urls.py  # Schema tenant
│   ├── celery.py
│   ├── asgi.py
│   └── wsgi.py
├── templates/
├── static/
├── media/
├── fixtures/
├── manage.py
├── requirements.txt
└── .env
```

---

## Commands úteis

```bash
# Listar tenants
python manage.py shell -c "from apps.tenants.models import TenantCompany; print(list(TenantCompany.objects.values('schema_name','name')))"

# Rodar qualquer comando em um tenant específico
python manage.py tenant_command <comando> --schema=<schema>

# Migrations
python manage.py migrate_schemas --shared   # schema público
python manage.py migrate_schemas --tenant   # todos os tenants
python manage.py migrate_schemas --schema=locadora1  # tenant específico
```

---

## Settings

O projeto usa `DJANGO_SETTINGS_MODULE=config.settings.development` por padrão. Para produção, crie `config/settings/production.py` herdando de `base.py` e ajuste `DEBUG`, `ALLOWED_HOSTS`, `STATICFILES_STORAGE` e as credenciais S3.
"# alugafacil" 
